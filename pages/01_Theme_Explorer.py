...
with st.expander(f'Theme Explorer Sunburst', expanded=True):
    ...
    with st.spinner(text='Doing my best...'):
...
with st.expander(f'Theme Explorer Table', expanded=True):
    ...
    #👇 Create spinner that displays "Loading..." while running.  Remember that everything contained on the spinner must be idented
    with st.spinner(text='Just a sec please...'):
        ...
        #                                                                              'color_name','is_trans'
        df_table_sets = df_filtered[['parent_theme_name', 'theme_name', 'year',
                                    'set_num', 'set_name', 'num_parts',
                                    'part_num', 'part_name', 'part_category_name',
                                    'quantity', 'color_name', 'is_trans']].drop_duplicates()
