...
filt_n_themes = st.slider ('Select a possible value for Top Master Themes', min_value=1, max_value=20, value=10)
...
with st.expander(f"New Sets of the Top {filt_n_themes} Master Themes", expanded=True):
    ...
    fig = px.bar(sets_per_parent_theme, x='nbr_sets', y='parent_theme_name', color='parent_theme_name',
             title= f'Number of Distinct Lego Sets per Parent Theme (Top {filt_n_themes})',color_discrete_sequence=sets_per_parent_theme.colors)
    fig.update_layout(yaxis_title=' ',
                  xaxis_title='Number of Different Sets')
...
#👇 Create an expander container widget with title "New Sets of the Top Master Themes"
with st.expander(f'New Sets of the Top {filt_n_themes} Master Themes over the years'):
...
with st.expander('Largest Lego Set per Year'): 
    ...
    largest_set_year = df_sets_to_theme_group.loc[df_sets_to_theme_group['rank'] == 1].sort_values('year', ascending=False)
    ...
    fig_largest_set_year.update_layout(xaxis_title='Year', 
                    yaxis_title='Number of New Parts per Set')
...
with st.expander('Free Table Explorer'):
